...
Bun.serve({
  ...
  routes: {
    ...
    // Sample piece models (glTF, CC0 — generated by scripts/export-models.ts).
    "/models/*": (req) => {
      const name = new URL(req.url).pathname.replace(/^\/models\//, "");
      if (name.includes("..")) return new Response("Not found", { status: 404 });
      const file = Bun.file(`${import.meta.dir}/models/${name}`);
      return file.exists().then((ok) => (ok ? new Response(file, { headers: { "content-type": "model/gltf+json" } }) : new Response("Not found", { status: 404 })));
    },
    "/assets/*": (req) => {
