...
import type { Analysis, AnalyzeOptions } from "./index";
...
export function analyzerClient(worker: Worker): AnalyzerClient {
  let nextId = 1;
  const pending = new Map<number, { resolve: (v: Analysis) => void; reject: (e: Error) => void }>();
  worker.onmessage = (e: MessageEvent<{ id: number; ok: boolean; value?: Analysis; error?: string }>) => {
  ...
  return {
    analyze: (scramble, options = {}) =>
      new Promise((resolve, reject) => {
        const id = nextId++;
        pending.set(id, { resolve, reject });
        const { known, ...rest } = options;
        worker.postMessage({ id, scramble: typeof scramble === "string" ? scramble : [...scramble], options: { ...rest, ...(known ? { known: [...known] } : {}) } });
      }),
    terminate: () => {
      worker.terminate();
