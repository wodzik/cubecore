...
| `@cubecore/skin` | skins as data: tile shapes, stickerless tiles, piece shapes, decals (PNG / SVG), features (`defineFeature`), glTF piece models, light / dark themes; presets `standard`, `stickerless`, `gan`, `qiyiSC`, `moyu` and their stickered versions (`withStickers`), matte / UV (`withFinish`) |
